...
def add_q(diff, topic, q_text, code, correct, fakes, explanation):
    ...
    while len(opts) < 5 and attempts < 100:
        if isinstance(correct, int):
            ...
            f = str(correct) + str(random.randint(1,9))
        if f not in opts:
            opts.append(f)
        attempts += 1
...
def gen_beg_theory():
    t = random.choice([
        ("변수", "파이썬에서 변수를 선언할 때 올바른 방법은?", "", "x = 10", ["int x = 10", "var x = 10", "10 = x"], "파이썬은 동적 타이핑을 지원하므로 '변수명 = 값' 형태로 선언합니다."),
        ("자료형", "다음 중 파이썬의 기본 자료형이 아닌 것은?", "", "double", ["int", "float", "str"], "파이썬에서 실수는 float이며 double이라는 키워드는 사용하지 않습니다."),
...
def gen_int_theory():
    t = random.choice([
        ("반복문", "while 문과 for 문의 설명으로 틀린 것은?", "", "for 문은 무한 루프를 만들 수 없다.", ["while 문은 조건이 참인 동안 반복한다.", "break를 사용해 탈출할 수 있다.", "continue를 사용해 다음 반복으로 넘어갈 수 있다."], "for문도 반복 가능한 무한 제너레이터를 사용하면 무한 루프를 구현할 수 있습니다."),
        ("함수", "여러 값을 한 번에 반환(return)할 때 기본적으로 어떤 자료형으로 묶여 반환되는가?", "", "튜플(Tuple)", ["리스트(List)", "딕셔너리(Dictionary)", "세트(Set)"], "여러 값을 콤마로 구분하여 반환하면 자동으로 튜플로 패킹됩니다."),
        ("모듈", "외부 모듈을 불러올 때 사용하는 파이썬 키워드는?", "", "import", ["include", "require", "using"], "파이썬에서는 import 키워드를 사용해 모듈을 가져옵니다.")
    ])
...
def gen_adv_theory():
    t = random.choice([
        ("클래스", "클래스 내부에 정의되어 객체 자신을 참조하는 첫 번째 매개변수의 관례적인 이름은?", "", "self", ["this", "cls", "object"], "파이썬에서는 인스턴스 메서드의 첫 번째 인자로 self를 사용하는 것이 관례입니다."),
        ("예외처리", "사용자가 직접 예외를 발생시키고자 할 때 사용하는 키워드는?", "", "raise", ["throw", "catch", "except"], "파이썬에서는 raise 키워드를 사용하여 의도적으로 예외를 발생시킵니다."),
        ("제너레이터", "제너레이터 함수에서 값을 반환하고 실행 상태를 유지하기 위해 사용하는 키워드는?", "", "yield", ["return", "emit", "give"], "yield 키워드는 값을 반환한 뒤 함수의 실행 상태를 저장하여 나중에 재개할 수 있게 합니다.")
    ])
...
def gen_exp_theory():
    t = random.choice([
        ("메타클래스", "파이썬에서 클래스 자체를 생성하는 클래스, 즉 클래스의 팩토리를 의미하는 용어는?", "", "메타클래스(Metaclass)", ["슈퍼클래스", "베이스클래스", "추상클래스"], "클래스가 객체를 생성하듯, 메타클래스는 클래스 자체를 생성합니다."),
        ("특수 메서드", "객체를 문자열로 표현할 때, 개발자 디버깅용 공식 문자열 표현을 반환하는 스페셜 메서드는?", "", "__repr__", ["__str__", "__format__", "__doc__"], "__repr__은 객체를 재생성할 수 있는 공식적인 문자열 표현을 제공하는 목적이 강합니다."),
        ("GIL", "CPython에서 여러 스레드가 파이썬 바이트코드를 동시에 실행하지 못하도록 하는 뮤텍스는?", "", "GIL", ["GC", "JIT", "MRO"], "GIL로 인해 CPython은 멀티코어 환경에서도 CPU 바운드 작업을 완벽히 병렬로 처리하는 데 한계가 있습니다.")
    ])
